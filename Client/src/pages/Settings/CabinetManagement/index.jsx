...
function CabinetManagement({ t }) {
  ...
  const fetchCabinets = async () => {
    ...
    try {
      ...
      console.log("Cabinets fetched:", response.data);
  ...
  const getStatusBadge = (status) => {
    if (status === "Online") {
      return (
        <Badge
          status="success"
          text={<span className="text-[1.4rem] text-white">Online</span>}
        />
      );
      ...
      return (
        <Badge
          status="error"
          text={<span className="text-[1.4rem] text-white">Offline</span>}
        />
      );
    } else {
      return (
        <Badge
          status="warning"
          text={<span className="text-[1.4rem] text-white">{status}</span>}
        />
      );
    }
  };
  ...
  const columns = [
    {
      title: (
        <span className="text-[1.6rem] font-semibold">{t("cabinet.name")}</span>
      ),
      dataIndex: "name",
      key: "name",
      render: (text) => (
    ...
    {
      title: (
        <span className="text-[1.6rem] font-semibold">{t("cabinet.type")}</span>
      ),
    ...
    },
    {
      ...
      dataIndex: "loops_supported",
      key: "loops_supported",
      render: (loops) => (
        <span className="text-[1.4rem] font-medium">{loops || 0} loops</span>
      ),
      width: 120,
      align: "center",
    },
