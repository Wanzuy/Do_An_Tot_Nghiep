...
function ZonesManagement({ t }) {
  ...
  // State cho modal gán thiết bị
  const [assignModalVisible, setAssignModalVisible] = useState(false);
  const [zoneToAssign, setZoneToAssign] = useState(null);
  ...
  const handleAssignModalClose = () => {
    ...
    setZoneToAssign(null);
  };

  const onAssignSuccess = () => {
    // Có thể refresh lại danh sách zones nếu cần hiển thị số lượng detector đã gán
    // hoặc để callback này trống nếu không cần cập nhật gì
  };
  ...
  const onEditSuccess = (updatedZone) => {
    setZones(
      zones.map((zone) =>
        zone._id === updatedZone._id ? { ...zone, ...updatedZone } : zone
      )
    );
  };
