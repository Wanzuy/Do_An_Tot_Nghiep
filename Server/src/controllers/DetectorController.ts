...
export const createDetector = async (req: any, res: any) => {
  try {
    ...
    const existingDetectorsCount = await DetectorModel.countDocuments({
...
export const getAllDetectors = async (req: any, res: any) => {
  try {
    ...
    if (req.query.detector_type) filter.detector_type = req.query.detector_type;
      ...
      if (!mongoose.Types.ObjectId.isValid(req.body.falcBoardId)) {
        ...
        if (!falcBoard) {
          return res.status(404).json({
            success: false,
            message:
              "Không tìm thấy Bo mạch FALC với ID mới " + req.body.falcBoardId,
          });
        }
      }
    }
...
export const updateDetectorStatus = async (req: any, res: any) => {
  try {
    ...
    if (
      ...
      const logStatus =
        updatedDetector.status === "Alarm" || updatedDetector.status === "Fault"
          ? "Active"
          : "Info";
      ...
      await createEventLog(
        ...
        updatedDetector.zoneId ? (updatedDetector.zoneId as any)._id : null, // ID Zone
        panelId, // ID Panel
