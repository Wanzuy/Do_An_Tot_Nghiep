...
export const getAIHealthStatus = async (req: any, res: any) => {
  try {
    const { getAPIHealthStatus, testAPIConnection } = await import(
      "../services/GeminiService"
    );

    const healthStatus = getAPIHealthStatus() as {
    ...
    if (testConnection && !healthStatus.circuitBreakerStatus.isOpen) {
    ...
    return res.status(500).json({
      success: false,
      message: "Không thể lấy thông tin trạng thái AI service",
    });
  }
};

// Get health recommendations based on current status
const getHealthRecommendations = (healthStatus: any): string[] => {
  const recommendations: string[] = [];

  if (healthStatus.circuitBreakerStatus.isOpen) {
    recommendations.push(
      "AI service tạm thời ngừng hoạt động - hệ thống sẽ tự động khôi phục"
    );
    recommendations.push(
      `Thời gian khôi phục dự kiến: ${Math.ceil(
        healthStatus.circuitBreakerStatus.timeUntilReset / 60000
      )} phút`
    );
    recommendations.push(
      "Sử dụng các chức năng quản lý khác trong thời gian chờ đợi"
    );
  } else if (healthStatus.circuitBreakerStatus.failureCount > 0) {
    recommendations.push(
      "AI service đang có một số vấn đề nhưng vẫn hoạt động"
    );
    recommendations.push("Khuyến nghị kiểm tra kết nối mạng");
  } else {
    recommendations.push("AI service hoạt động bình thường");
  }
