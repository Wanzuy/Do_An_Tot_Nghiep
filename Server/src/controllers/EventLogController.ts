        ...
        if (req.query.zoneId) {
            if (!mongoose.Types.ObjectId.isValid(req.query.zoneId)) {
                return res.status(400).json({
                    success: false,
                    message:
                        "Định dạng ID Zone trong tham số truy vấn không hợp lệ.",
                });
            }
        ...
        if (req.query.panelId) {
            if (!mongoose.Types.ObjectId.isValid(req.query.panelId)) {
                return res.status(400).json({
                    success: false,
                    message:
                        "Định dạng ID Panel trong tham số truy vấn không hợp lệ.",
                });
            }
    ...
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
            return res.status(404).json({
                success: false,
                message:
                    "Không tìm thấy bản ghi sự kiện với ID " + req.params.id,
            });
        }
