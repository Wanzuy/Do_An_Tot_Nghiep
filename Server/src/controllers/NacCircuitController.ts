      ...
      if (!mongoose.Types.ObjectId.isValid(req.body.nacBoardId)) {
        ...
        if (!nacBoard) {
          return res.status(404).json({
            success: false,
            message:
              "Không tìm thấy Bo mạch NAC với ID mới " + req.body.nacBoardId,
          });
        }
      }
    }
...
export const deleteNacCircuit = async (req: any, res: any) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        error.message || "Không thể xóa Mạch NAC với ID " + req.params.id,
    });
  }
};
 ...
 */
export const getCircuitsByZoneId = async (req: any, res: any) => {
  try {
    // Validate zone ID
    ...
    const circuits = await NacCircuitModel.find({ zoneId: req.params.zoneId })
