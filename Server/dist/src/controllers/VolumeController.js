...
exports.testVolume = exports.updateVolumeSetting = exports.getVolumeSettingByPanel = exports.createVolumeSetting = void 0;
 ...
 */
const createVolumeSetting = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const panel = yield PanelModel_1.default.findById(panelId);
        if (!panel) {
        ...
        const existingVolume = yield VolumeModel_1.default.findOne({ panelId });
        if (existingVolume) {
            return res.status(400).json({
                success: false,
                message: "Panel này đã có cài đặt âm lượng. Mỗi panel chỉ có một cài đặt âm lượng.",
            });
        }
        // Validate level
        ...
        const newVolume = new VolumeModel_1.default({ panelId, level });
        const savedVolume = yield newVolume.save();
        // Populate panelId
        const result = yield VolumeModel_1.default.findById(savedVolume._id).populate("panelId", "name panel_type");
        res.status(201).json({
            success: true,
        ...
        if (error.name === "ValidationError") {
            ...
            res.status(500).json({
                success: false,
                message: error.message || "Đã xảy ra lỗi khi tạo cài đặt âm lượng.",
            });
        }
    }
});
exports.createVolumeSetting = createVolumeSetting;
/**
 * Get volume settings by panel ID
 */
const getVolumeSettingByPanel = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const volume = yield VolumeModel_1.default.findOne({ panelId }).populate("panelId", "name panel_type");
        if (!volume) {
            return res.status(404).json({
                success: false,
...
exports.getVolumeSettingByPanel = getVolumeSettingByPanel;
/**
 * Update volume setting by ID
 */
const updateVolumeSetting = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        ...
        const updatedVolume = yield VolumeModel_1.default.findByIdAndUpdate(id, { level }, { new: true, runValidators: true }).populate("panelId", "name panel_type");
        if (!updatedVolume) {
            return res.status(404).json({
                success: false,
                message: "Không tìm thấy cài đặt âm lượng với ID " + id,
        ...
        if (error.name === "ValidationError") {
            ...
            return res.status(404).json({
                success: false,
                message: "Không tìm thấy cài đặt âm lượng với ID " + req.params.id,
            });
        }
        else {
            res.status(500).json({
...
exports.updateVolumeSetting = updateVolumeSetting;
/**
 * Test volume setting
 */
const testVolume = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        ...
        // Tìm volume setting
        const volume = yield VolumeModel_1.default.findById(id).populate("panelId", "name panel_type");
        if (!volume) {
            return res.status(404).json({
                success: false,
                message: "Không tìm thấy cài đặt âm lượng với ID " + id,
        ...
        // Validate testLevel
        const testLevelValue = testLevel !== undefined ? testLevel : volume.level;
        if (typeof testLevelValue !== "number" ||
            testLevelValue < 0 ||
            testLevelValue > 100) {
            return res.status(400).json({
                success: false,
